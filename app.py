...
# Deep Executive Aesthetics Styling
st.markdown("""
    <style>
    .report-box {
...
# Explicit setup mapping the options to matching file strings in your checkpoints directory
CASE_MAPPING = {
...
def locate_checkpoint_file(filename: str):
    """
    Scans the directories defensively to accommodate variations between 
    local machines and GitHub cloud hosting pathways.
    """
    possible_paths = [
        Path(".") / "checkpoints" / filename,
        Path(".") / "pod-fcdnn-app-main" / "checkpoints" / filename,
        Path(".") / filename
    ]
...
def agent_tool_reconstruct_field(case_name: str, reynolds: float):
    ...
    # 1. Run inference using code from engine.py
    trainer = load_checkpoint(checkpoint_path, device=device)
    x_coords, y_coords, u, v = predict_and_reconstruct(trainer, reynolds)
    
    # 2. Resampling Interpolation Step for High-Fidelity Contours
...
with col_left:
    ...
    if process_btn:
        # Defaults parsing simulation values
        target_case = "Cylinder Wake Vortex"
        target_re = 200.0
        
        # Match case statements based on keyword prompt strings 
        ...
        # Simple extraction numbers array capture logic
        ...
        # Numerical boundary validator checks
        ...
        • Fetching neural tensors, processing weights, and running matrix transformations...
        </div>
        """, unsafe_allow_html=True)
        ...
        try:
            ...
            • Reconstructed spatial configurations utilizing <span class='code-span'>{active_modes}</span> active POD dimensions.<br>
            • Field coordinates array matrices mapped seamlessly onto continuous mesh layer.
            </div>
            """, unsafe_allow_html=True)
        ...
        st.write("Awaiting engineering commands stream...")
...
with col_right:
    ...
    if 'results' in st.session_state and process_btn:
        ...
        # High-Fidelity Plotly Continuous Heat Contours
        fig_u = go.Figure(data=[
            go.Contour(
                ...
                colorbar=dict(title="U Field Magnitude", titleside="top")
            )
        ])
        fig_u.update_layout(
            title=f"AI Contour Field (Horizontal U Fluid Direction) | Re = {t_re}",
            xaxis_title="X Coordinates", yaxis_title="Y Coordinates",
        ...
        fig_v = go.Figure(data=[
            go.Contour(
                ...
                line_width=0.2,
                contours=dict(coloring='heatmap', showlines=True),
                colorbar=dict(title="V Field Magnitude", titleside="top")
            )
        ])
        fig_v.update_layout(
